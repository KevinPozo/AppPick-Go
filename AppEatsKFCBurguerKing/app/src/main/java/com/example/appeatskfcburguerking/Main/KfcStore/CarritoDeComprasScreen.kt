...
@Composable
fun CarritoDeComprasScreen(carritoViewModel: CarritoViewModel, navController: NavController) {
    val productos = carritoViewModel.productosSeleccionados
    val opciones = carritoViewModel.opcionesSeleccionadas
    val total = carritoViewModel.calcularTotal()
    val historialDePedidos = carritoViewModel._historialDePedidos
    ...
    Scaffold(
        ...
        bottomBar = {
            Box(
                ...
                Button(
                    onClick = {
                        navController.navigate("realizarpagocarrito")
                    },
                    modifier = Modifier.fillMaxWidth(),
                    ...
                    Column(modifier = Modifier.padding(16.dp)) {
                        ...
                        ultimoPedido.opciones.forEach { opcion ->
                            if (opcion.tipo == TipoDeOpcion.PRESA) {
                                Text("Tipo de Presa: ${opcion.nombre}", style = MaterialTheme.typography.bodySmall)
                            } else {
